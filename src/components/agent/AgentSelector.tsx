...
import { useTheme } from '../../contexts/ThemeContext';
import { useAgent } from '../../contexts/AgentContext';
import { Sparkles, ArrowRight, Bot, Zap, TrendingUp } from 'lucide-react';
...
export const AgentSelector: React.FC = () => {
  ...
  const handleStartChat = () => {
    // "chat" view will trigger the WebSocket connection in the Chat Component
    setCurrentView('chat'); 
  };

  return (
    <div className="h-full flex flex-col p-6 overflow-y-auto" style={{ backgroundColor: currentTheme.colors.background }}>
      ...
      <div className="max-w-4xl mx-auto w-full mb-8">
        ...
        <p className="opacity-70" style={{ color: currentTheme.colors.textSecondary }}>
          Your personal data analyst and store manager.
        </p>
      </div>
      ...
      <div className="max-w-4xl mx-auto w-full flex-1 flex flex-col justify-center">
        ...
        <div 
          className="group relative overflow-hidden rounded-3xl border transition-all duration-500 hover:shadow-2xl cursor-pointer"
          onClick={handleStartChat}
          style={{
            background: `linear-gradient(135deg, ${currentTheme.colors.surface}, ${currentTheme.colors.background})`,
            borderColor: currentTheme.colors.primary + '40', // Low opacity border
            boxShadow: `0 10px 30px -10px ${currentTheme.shadows.primary}`
          }}
        >
          {/* Animated Gradient Background */}
          <div className="absolute inset-0 opacity-0 group-hover:opacity-10 transition-opacity duration-700 bg-gradient-to-r from-transparent via-white to-transparent transform -skew-x-12 translate-x-[-100%] group-hover:translate-x-[100%]" />

          <div className="p-8 sm:p-12 flex flex-col md:flex-row items-center gap-8 text-center md:text-left">
            
            {/* Avatar Section */}
            <div className="relative shrink-0">
              <div className="w-24 h-24 sm:w-32 sm:h-32 rounded-full flex items-center justify-center shadow-lg relative z-10"
              ...
              {/* Pulse Effect */}
              <div className="absolute inset-0 rounded-full animate-ping opacity-20" style={{ backgroundColor: currentTheme.colors.primary }} />
            </div>

            {/* Content Section */}
            <div className="flex-1">
              <h2 className="text-2xl sm:text-3xl font-bold mb-3" style={{ color: currentTheme.colors.text }}>
                Amesie Intelligence
              </h2>
              <p className="text-lg mb-6 leading-relaxed opacity-80" style={{ color: currentTheme.colors.text }}>
                I can help you analyze sales, manage inventory, and predict trends. Just ask "How are my sales today?" or "Update stock for item #5".
              </p>

              {/* Capabilities Pills */}
              <div className="flex flex-wrap justify-center md:justify-start gap-3 mb-8">
                <span className="px-4 py-2 rounded-full text-xs font-bold uppercase tracking-wider flex items-center gap-2 bg-slate-100 text-slate-600">
                  <TrendingUp size={14} /> Sales Analysis
                </span>
                <span className="px-4 py-2 rounded-full text-xs font-bold uppercase tracking-wider flex items-center gap-2 bg-slate-100 text-slate-600">
                  <Zap size={14} /> Instant Actions
                </span>
                <span className="px-4 py-2 rounded-full text-xs font-bold uppercase tracking-wider flex items-center gap-2 bg-slate-100 text-slate-600">
              ...
              <button 
                ...
                className="inline-flex items-center space-x-2 px-8 py-4 rounded-xl font-bold text-white transition-transform hover:scale-105 active:scale-95 shadow-lg"
                style={{ background: `linear-gradient(135deg, ${currentTheme.colors.primary}, ${currentTheme.colors.secondary})` }}
              >
