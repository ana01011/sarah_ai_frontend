...
import { Search, Plus, Filter, X, Upload, Image as ImageIcon } from 'lucide-react';
...
const MenuSkeletonCard = () => (
  <div className="bg-white p-4 rounded-2xl border border-slate-100 shadow-sm space-y-4">
    ...
    <div className="space-y-2">
      <Skeleton height={24} width="70%" /> {/* Title */}
      <Skeleton height={16} width="90%" /> {/* Desc */}
    </div>
...
export const AmesieMenu: React.FC = () => {
  ...
  const handleAddItem = async (e: React.FormEvent) => {
    ...
    setSubmitting(true);
    try {
      // NOTE: Ensure your amesieService.addMenuItem is updated to handle { ...item, images: File[] }
  ...
  return (
    <div className="h-full flex flex-col bg-slate-50 relative">
      ...
      <div className="bg-white border-b sticky top-0 z-10" style={{ borderColor: currentTheme.colors.border }}>
        <div className="px-6 py-4 flex flex-col space-y-4">
          <div className="flex justify-between items-center">
            <h2 className="text-2xl font-bold text-slate-800 pl-8">Menu Items</h2>
            <button 
      ...
      <div className="flex-1 overflow-y-auto p-6">
        {loading ? (
          // --- UPDATED: SKELETON GRID ---
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
             {/* Render 8 dummy skeletons while loading */}
             {[...Array(8)].map((_, i) => (
      ...
      {isModalOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4 backdrop-blur-sm">
          <div className="bg-white rounded-2xl w-full max-w-md p-6 shadow-2xl relative animate-in fade-in zoom-in-95 max-h-[90vh] overflow-y-auto">
            <button 
